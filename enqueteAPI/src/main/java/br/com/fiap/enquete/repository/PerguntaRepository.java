...
public class PerguntaRepository {
    ...
    public int update(Pergunta pergunta) throws SQLException {
        String sql = "UPDATE k_pergunta SET enunciado=?, numero=?, tipo=? WHERE id = ?";
        PreparedStatement pstmt = this.con.prepareStatement(sql);
        pstmt.setString(1, pergunta.getEnunciado());
        pstmt.setInt(2, pergunta.getNumero());
        pstmt.setString(3, pergunta.getTipo().name());
        pstmt.setLong(4, pergunta.getId());

        int alterados = pstmt.executeUpdate();
        pstmt.close();
        return alterados;
    }
}
